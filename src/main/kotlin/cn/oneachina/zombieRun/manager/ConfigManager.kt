...
class ConfigManager(private val plugin: ZombieRun) {
    ...
    fun getAlphaZombieHealthMultiplier(): Double {
        return config.getDouble("alpha-zombie.health-multiplier", 25.0)
    }

    fun getAlphaZombieParticleEnabled(): Boolean {
        return config.getBoolean("alpha-zombie.particle", true)
    }
